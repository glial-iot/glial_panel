<template lang="html">
  <div>
    <v-btn v-if="visual !== 'none'" color="secondary" class="my-2" @click="dialog_visible = true">
      ...
      Create {{$options.filters.type2string(type)}}
    </v-btn>

    <v-dialog v-on:keydown.esc="close()" v-on:keydown.enter="create_script()" v-model="dialog_visible"
              max-width="500px">
      <v-card>
        <v-card-title>
        ...
        <v-card-text v-if="type != 'DRIVER'" class="py-0">
          <v-text-field @keyup="$refs.matchingTopicsList.get_mask_match()" :label="$options.filters.object_label(type)"
                        v-model="object"></v-text-field>
        </v-card-text>
        <bus-event-shortcuts class="pt-0" v-if="type === 'BUS_EVENT'"
                             v-on:set_object="object = $event; $refs.matchingTopicsList.get_mask_match()"></bus-event-shortcuts>
        <matching-topics-list ref="matchingTopicsList" :mask="object"
                              v-if="type === 'BUS_EVENT'"></matching-topics-list>
        <timer-event-shortcuts class="pt-0" v-if="type === 'TIMER_EVENT'"
                               v-on:set_object="object = $event"></timer-event-shortcuts>
        <schedule-event-shortcuts class="pt-0" v-if="type === 'SHEDULE_EVENT'"
                                  v-on:set_object="object = $event"></schedule-event-shortcuts>
        <schedule-event-crontab-description class="pt-0"
                                            v-if="type === 'SHEDULE_EVENT'"></schedule-event-crontab-description>
