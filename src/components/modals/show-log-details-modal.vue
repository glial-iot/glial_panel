    ...
    <v-dialog v-on:keydown.esc="visible = false" v-model="visible" max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" flat v-if="visible_trace === false" @click.stop="visible_trace = true">Show trace
          </v-btn>
          <v-btn color="primary" flat v-if="visible_trace === true" @click.stop="visible_trace = false">Hide trace
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
