            ...
            <v-form left>
                <v-layout row wrap mt-3 pt-3>
                    <v-flex xs4>
                        ...
                        <v-title>Setting # 2 (reserved for a future use)</v-title>
                    </v-flex>
                    <v-flex xs5>
                        <v-text-field name="input-3-2" label="Setting # 2" v-model="server_url">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs3>
                        <v-btn :loading="reserved_button.loading" :disabled="reserved_button.disabled" :color="reserved_button.color">
                            <v-icon left small>fa-arrow-alt-circle-right</v-icon> Update
                        </v-btn>
                    </v-flex>
                </v-layout>
        ...
        <v-card>
            <v-form left>
                <v-layout row wrap mt-3 pt-3>
                    <v-flex xs4>
                        <v-icon>fa-wrench</v-icon>
                        <v-title>Setting #3 (reserved for a future use)</v-title>
                    </v-flex>
                    <v-flex xs5>
                        <v-text-field name="input-3-3" label="Setting #3" value="123.123.123.123">
                        </v-text-field>
                    </v-flex>
                    <v-flex xs3>
                        <v-btn :loading="reserved_button.loading" :disabled="reserved_button.disabled" :color="reserved_button.color">
...
export default {
  ...
  methods: {
    url_update() {
      ...
      this.$store.commit("update", this.server_url);
